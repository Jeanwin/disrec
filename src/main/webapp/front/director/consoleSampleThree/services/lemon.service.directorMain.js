    ...
    angular.module('lemon.service.directorMain', [])
        .constant('DirectorMainServiceConfig', {
            ...
            urls:{
				...
				newrtspState:config.backend.ip + config.backend.base2 +'queryRecordMode',
				newrtspPreview:config.backend.ip + config.backend.base2 +'sendToRecord',
				...
				sendCentralControlCmd:config.backend.ip + config.backend.base +'sendCmdToCentralControl?Cmd='
            }
        })
...
//                    var _url=DirectorMainServiceConfig.urls.livingStatus;
				...
				initNewRtspPreview:function(ip){
                    var deffered=$q.defer();
                    var _url = DirectorMainServiceConfig.urls.newrtspPreview + "?ip=" +ip;
					console.log(_url)
                    $http({
                        method: 'GET',
                        dataType:"json",
				...
				initNewRtspState:function(ip){
                    var deffered=$q.defer();
                    var _url = DirectorMainServiceConfig.urls.newrtspState + "?ip=" +ip;
                    $http({
                        method: 'GET',
                        dataType:"json",
                        contentType:"application/json:charset=UTF-8",
				...
				*/
				sendCentralControlCmd : function(cmd,ip){
					var deffered = $q.defer();
                    var _url = DirectorMainServiceConfig.urls.sendCentralControlCmd  + cmd +'&ips=' + ip;
                    $http({
                        method: config.data.method==='files'? 'GET':'GET',
                        dataType: "json",
                        contentType:'application/json; charset=UTF-8',
