define(['app','config'], function (app,config) {
    app.registerController('SystemServerConfigureCtrl', ['$scope','$modal','$location','SystemService','TacticsService','$timeout',
        ...
        var ApplyConsoleModalCtrl=function($scope,$modalInstance,growl,deviceLists){

            $scope.deviceLists=deviceLists;
            ...
            $scope.saveApply=function(index){
                console.log($scope.deviceLists);
                console.log($scope.deviceLists.length);
                var keywords=[];
                for(var i=0;i<$scope.deviceLists.length;i++){
        ...
        var init = function(){
            ...
            $scope.deviceSearch={"name":""};
            $scope.showDeviceServerList($scope.deviceSearch);
        };
        init();
    }]);
});