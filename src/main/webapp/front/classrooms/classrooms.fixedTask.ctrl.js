define(['app',
    ...
    app.registerController('FixedTaskCtrl', ['$scope','$modal','$location','$upload','growl','FacilityService' ,'TacticsService',
        '$http', '$filter', 'TreeService','$timeout','ClassroomService',
        function ($scope,$modal,$location,$upload,growl,FacilityService ,TacticsService,$http, $filter,TreeService,$timeout,ClassroomService) {
    	
    	//获取设备类型
    	var FsystenCode = function(){
    		ClassroomService.GetfacilityCode().then(
                function(data){
                    $scope.FsystenCode = data;
                })
    	}
    	...
    	$scope.taskList = function(){
    		ClassroomService.FixedTaskList($scope.pagination).then(
    			function(data){
    				$scope.taskLists = data.data;
    	...
    	$scope.deleteTask = function(index){
    		var modalInstance = $modal.open({
                templateUrl: 'classrooms/task/classrooms.taskDelete.modal.html',
                backdrop:'static',
                controller: DeleteTaskModalCtrl,
                resolve: {
                	taskID:function(){
                		return $scope.taskLists[index].id;
                	}
    	...
    	var DeleteTaskModalCtrl = function ($scope, $modalInstance,growl,taskID) {
    		var tasktransfer = [];
    		tasktransfer.push({id:taskID});
    		$scope.cancel = function(){
    			$modalInstance.dismiss('cancel');
    		}
    		$scope.save = function(){
    			ClassroomService.DeleteFixedTask(tasktransfer).then(
            			function(data){
                			if(data == '1'){
       ...
       //修改定时任务
        $scope.modifyTask = function(opera,index){
        	var transferdata;
        	if(opera == 'edit'){
        		transferdata = $scope.taskLists[index];
        	}
             var modalInstance = $modal.open({
                templateUrl: 'classrooms/task/classrooms.task.modal.html',
                backdrop:'static',
                ...
                resolve: {
                	...
                	FsystenCode:function(){
                		return $scope.FsystenCode;
                	}
                }
            }).result.then(
                    function(){
        ...
        var ModifyTaskModalCtrl = function ($scope, $modalInstance,ClassroomService,$location,growl,areaTree,transferdata,opera,FsystenCode) {
        	$scope.areaTree = angular.copy(areaTree);
        	$scope.FsystenCode = FsystenCode;
        	if(opera === 'edit'){
        		$scope.task = {
            			'id':transferdata.id,
            			"taskType": transferdata.taskType,
            			"execStart": transferdata.execStart,                         
            			"execEnd": transferdata.execEnd,                               
            			"weekMap": transferdata.weekMap,
            			'startTime':transferdata.startTime,
            			'areas':transferdata.areas
            			
            		}
        		$scope.task.taskTypetemp = transferdata.taskName + ',' + transferdata.taskType;
        	}else{
        		$scope.task = {
        				"taskTypetemp":"",
            			"taskType": "",                                  
        	...
        	$scope.splitData = function(data){
        		$scope.task.taskType = data.split(',')[1];
        	}
            ...
            $scope.save = function(){
            	delete $scope.task.taskTypetemp;
            	getSelectClassrooms($scope.areaTree[0]);
            	if(opera === 'edit'){
	            	ClassroomService.UpdateFixedTask($scope.task).then(
	            			function(data){
	            				if(data == '1'){
	                				$modalInstance.close(true);
	                				growl.addSuccessMessage("新建定时任务成功");
	            				}
	            				if(data == '0'){
	            					$modalInstance.dismiss('cancel');
	                				growl.addErrorMessage("新建定时任务失败");
	            				}
	            	...
	            	ClassroomService.CreateFixedTask($scope.task).then(
	            			function(data){
	            				if(data == '1'){
	                				$modalInstance.close(true);
	                				growl.addSuccessMessage("新建定时任务成功");
            ...
            var inScope = function(id){
                ...
                $.each($scope.task.areas, function(index, classroomScope){
...
//                                    _node.isSelected = !(_node.isSelected);
        ...
        var init = function(){
           ...
           //判断是否有权限
            if($scope.global.user.authenticatid.indexOf('auth_time_tark_url_view') === -1){
                console.log("+++++++教室设置  您没有权限+++++++++");
                $location.path('classrooms/cardSet');
            };
