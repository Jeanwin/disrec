define(['app',
    ...
    app.registerController('ConfigFacilityCtrl', ['$scope','$modal','$stateParams','$location','growl' ,'TacticsService',
        '$http', '$filter', 'TreeService','$stateParams','TreeService','$timeout','SystemService',
        function ($scope,$modal,$stateParams,$location,growl ,TacticsService,$http, $filter,TreeService,$stateParams,TreeService,$timeout,SystemService) {
       ...
       /*                         $scope.alarmList[i].outputName="null";
        ...
        $scope.mainTrees = function(keywords,areaid){
            SystemService.showDeviceServerList().then(
                function(data){
        ...
        //声明变量来控制教室的多选、全选、反选。初始化全选为false
        ...
        $scope.initChkItem=function(){
            ...
            if($scope.check.chkItem.length==0 && $scope.areaTree.length>0){
        ...
        $scope.clickAllObj=function(flag){
        	...
        	$scope.save = function(){
        		...
        		getSelectClassrooms($scope.areaTree);
                /*for(var i=0;i<$scope.alarmList.length;i++){
                   if(){
        	...
        	//获取选中的节点
            var getSelectClassrooms = function(nodes){
                    for(var i=0;i<$scope.check.chkItem.length;i++){
                        if ($scope.check.chkItem[i]) {
                            $scope.areas.push(
                                {
