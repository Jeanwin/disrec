define(['app','config'], function (app,config) {
    app.registerController('MyresourceReleasedCtrl', ['$scope','$interval','$modal','$filter','$location','ResourceService' ,'TacticsService', function ($scope,$interval,$modal,$filter,$location,ResourceService,TacticsService) {
    	//开启点播状态
        ...
        $scope.openVideoSetModel = function (video) {
            ...
            var modalInstance = $modal.open({
                templateUrl: 'resource/video/resource.myvideo.modal.html',
                backdrop:'static',
...
//                        alert("删除失败!");
            ...
            /*确认删除功能*/
            $scope.ok=function(){
                $scope.deleteVideos($scope.selectedItems, {});
            };
...
//                        alert("删除失败!");
      ...
      //视频   取消发布按钮
        ...
        $scope.resourcevideos = function(select) {
            ...
            ResourceService.myresourceVideo($scope.videoResource,$scope.pagination,"").then(
                    function(data){
...
//            ];
            ...
            $scope.resourcevideos("",$scope.pagination,"");
            //判断是否有权限
            if($scope.global.user.authenticatid.indexOf('auth_resource_myresource_url_view') === -1){
//                alert("对不起！您没有权限访问。")
                console.log("+++++++视频资源  您没有权限+++++++++")
                $location.path('resource/recycle');
            };
