...
/**
 * @Title: @{#} SysMessageRestController.java
 * @Description: <p>SysMessage的Restful API的Controller.</p>
 * @author <a href="mailto:cuiwx@zonekey.com.cn">cuiwx</a>
 * @date 2014年9月20日 下午7:37:47
 * @version v 1.0
 */
@RestController
@RequestMapping(value = "/rest/sysmessage")
public class SysMessageRestController {
	private static final Logger LOG = LoggerFactory.getLogger(SysMessageRestController.class);
	...
	public Map<String, Object> page(@PathVariable("no") String no, @PathVariable("size") String size) {
		Page<SysMessage> dataPage = sysmessageService.findPageBy(Integer.parseInt(no), Integer.parseInt(size));
	...
	public ResponseEntity<?> create(@RequestBody SysMessage sysmessage, UriComponentsBuilder uriBuilder) {
		// 调用JSR303 Bean Validator进行校验, 异常将由RestExceptionHandler统一处理.
		BeanValidators.validateWithException(validator, sysmessage);
		...
		URI uri = uriBuilder.path("/sysmessage/" + id).build().toUri();
	...
	public void update(@RequestBody SysMessage sysmessage) {
		// 调用JSR303 Bean Validator进行校验, 异常将由RestExceptionHandler统一处理.
		BeanValidators.validateWithException(validator, sysmessage);
